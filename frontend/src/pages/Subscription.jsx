...
export function Subscription() {
  ...
  const canAddExtra = sel?.slug === "premium-3";
    ...
    <div className="subscription-page">
      ...
      <div className="current-plan-card">
        <div>
          ...
          <b>{user.planSlug === "trial" ? "Trial Gratis" : user.planSlug === "premium-1" ? "Premium 1" : user.planSlug === "premium-3" ? "Premium 3" : "Trial"}</b>
          <span>
      ...
      <div className="subscription-grid">
        {plans.map(p => (
          <div data-testid={`subscription-plan-${p.slug}`} className={`subscription-card ${selected === p.slug ? "selected" : ""}`} key={p.slug}>
            <span className="plan-kicker">{p.slug === "premium-3" ? "PALING DIPILIH" : "PAKET"}</span>
            <h2>{p.name}</h2>
            <div className="subscription-price">Rp{money(p.monthlyPrice)}<small>/bulan</small></div>
            <p>Hingga {p.websiteLimit} website bisnis</p>
...
export function PaymentFlow() {
  ...
  const total = plan.monthlyPrice + extra * (settings.additionalWebsitePrice || 25000);
  ...
  const submit = async () => {
    ...
    setBusy(true); setErr("");
    try {
      const r = await api.post("/payments", { planSlug: plan.slug, additionalWebsiteCount: extra, transferDate, proofUrl, notes });
      setPayment(r.data);
      setStep(3);
  ...
  const wa = () => {
    const msg = `Halo Admin UsahaKu, saya sudah melakukan pembayaran paket ${plan.name} sebesar Rp${money(total)}. Email akun saya: ${user.email}. Saya akan mengirimkan bukti transfer.`;
      ...
      {step === 1 && (
        <div className="wizard-card">
          ...
          <div className="bank-info">
            ...
            <div><small>TOTAL TRANSFER</small><b data-testid="payment-amount">Rp{money(total)}</b></div>
          </div>
      ...
      {step === 3 && payment && (
        <div className="wizard-card">
          <div className="success-panel">
            ...
            <p>Pembayaran <b>{plan.name}</b> senilai <b>Rp{money(total)}</b> sedang menunggu verifikasi admin. Kami akan memberi kabar melalui notifikasi.</p>
            <div className="wizard-actions">

    ...
    {products.map((product, index) => {
      ...
      return <article className="tpl-product" key={product.id || index}>
        <div className="tpl-product-image">
          {image ? <img src={image} alt={getImageAlt({ alt: product.imageAlt, context: product.name, businessName: data.businessName })} loading="lazy" width="480" height="360" /> : <ShoppingBag size={28} />}
        </div>
...
const WarmLayout = ({ data, c, bgUrl, coverStyle, products, fixUrl, wa, highlights, testimonials, articleUrl }) => (
  <div className="tpl-site tpl-warm">
    <header className="tpl-nav"><b>{data.businessName}</b><nav><a href="#cerita">Cerita kami</a><a href="#menu">Menu</a><a href="#ulasan">Ulasan</a>{articleUrl && <a href={articleUrl}>Artikel</a>}</nav><a className="tpl-nav-cta" href={wa} target="_blank" rel="noreferrer">Reservasi</a></header>
    <main>
      <section className="warm-hero"><div className="warm-copy"><small>{c.heroBadge || "RASA YANG BERKESAN"}</small><h1>{c.heroTitle || `Selamat datang di ${data.businessName}`}</h1><p>{c.heroSubtitle || data.description}</p><a href="#menu">Lihat pilihan <ArrowRight size={16} /></a></div><img src={bgUrl} alt={getImageAlt({ alt: data.coverImageAlt, context: "Tampilan usaha", businessName: data.businessName })} width="1600" height="900" fetchPriority="high" style={{ objectPosition: coverStyle?.objectPosition, filter: coverStyle?.filter }} /></section>
...
const MinimalLayout = ({ data, c, bgUrl, coverStyle, products, fixUrl, wa, testimonials, articleUrl }) => (
  <div className="tpl-site tpl-minimal-new"><header className="minimal-nav"><b>{data.businessName}</b><span>{data.category}</span>{articleUrl && <a href={articleUrl}>Artikel</a>}<a href={wa} target="_blank" rel="noreferrer">Kontak</a></header>
    <main><section className="minimal-hero"><div><small>EST. {data.city || "INDONESIA"}</small><h1>{c.heroTitle || data.businessName}</h1><p>{c.heroSubtitle || data.description}</p><a href="#koleksi">Eksplor koleksi <ArrowRight size={15} /></a></div><figure><img src={bgUrl} alt={getImageAlt({ alt: data.coverImageAlt, context: "Tampilan usaha", businessName: data.businessName })} width="1600" height="900" fetchPriority="high" style={{ objectPosition: coverStyle?.objectPosition, filter: coverStyle?.filter }} /><figcaption>{c.heroBadge || "Pilihan berkualitas untuk keseharian Anda"}</figcaption></figure></section>
...
const PlayfulLayout = ({ data, c, bgUrl, coverStyle, products, fixUrl, wa, highlights, testimonials, articleUrl }) => (
  <div className="tpl-site tpl-playful-new"><header className="playful-nav"><b>✦ {data.businessName}</b><span>{articleUrl && <a href={articleUrl}>Artikel</a>}<a href={wa} target="_blank" rel="noreferrer">Pesan sekarang!</a></span></header><main>
